...
"""
SlotMAX for Division 35. Same logic; 4 brands: CREMAFFIN PLUS, DIGERAFT PLUS, CREMAFFIN, LIBRAX.
Output: Report month (Feb) in template schema -> out_jan_div35.csv.

What it does:
1) Only 4 brands count: CREMAFFIN PLUS, DIGERAFT PLUS, CREMAFFIN, LIBRAX. All other brands ignored.
...
6) Pivot to product columns (CREMAFFIN PLUS, DIGERAFT PLUS, CREMAFFIN, LIBRAX).
...
ALLOWED_BRANDS = ["CREMAFFIN PLUS", "DIGERAFT PLUS", "CREMAFFIN", "LIBRAX"]
DEFAULT_INPUT = PROJECT_DIR / "DCR_RAW_STANDARDIZED_4div_2026-03-01_2026-03-31_Div35.xlsx"
DEFAULT_OUTPUT = PROJECT_DIR / "out_jan_div35.csv"
...
def get_baseline_months(report_jan: pd.Period) -> List[pd.Period]:
    """Return the three calendar months immediately before report_jan."""
...
def add_variance_flags_octdec_vs_jan(
    ...
    """
    Variance flags: report month total Rx vs average of the three baseline months (4 brands only).
    - Baseline = avg(month0, month1, month2) monthly totals per doctor; Sum/AVG columns use those three months.
    - If |report - avg_baseline| / avg_baseline > 25% then Variance >25% = TRUE (with zero-safe rules).
    Flags are set only on report-month rows; other months get False.
    """
    out = df.copy()
    for c in product_cols:
        if c in out.columns:
    ...
    oct_period, nov_period, dec_period = (
        baseline_months[0],
        baseline_months[1],
        baseline_months[2],
    )
    ...
    oct_totals = (
        out.loc[out[monthperiod_col] == oct_period]
    ...
    # Reuse baseline and report totals; safe variance logic (avoid division by zero)
    avg_baseline = (oct_totals + nov_totals + dec_totals) / 3.0
    report_total = jan_totals
    mask_both_zero = (avg_baseline == 0) & (report_total == 0)
    mask_baseline_zero_report_positive = (avg_baseline == 0) & (report_total > 0)
    percent_diff = np.where(
        mask_both_zero,
        0.0,
        np.where(
            mask_baseline_zero_report_positive,
            np.nan,
            ((report_total - avg_baseline) / avg_baseline) * 100.0,
        ),
    )
    variance_ser = pd.Series(
        np.where(
            mask_both_zero,
            False,
            np.where(mask_baseline_zero_report_positive, True, np.abs(percent_diff) > 25.0),
        ),
        index=jan_totals.index,
    )
    increasing_ser = pd.Series(
        np.where(
            mask_both_zero,
            False,
            np.where(mask_baseline_zero_report_positive, True, percent_diff >= 25.0),
        ),
        index=jan_totals.index,
    )
    decreasing_ser = pd.Series(
        np.where(
            mask_both_zero,
            False,
            np.where(mask_baseline_zero_report_positive, False, percent_diff <= -25.0),
        ),
        index=jan_totals.index,
    )
    all_zero_ser = (report_total == 0)

    sum_last_3_months = oct_totals + nov_totals + dec_totals
    out["Sum of last 3 months"] = out[doctor_col].map(lambda d: sum_last_3_months.get(d, 0))
    out["AVG of last 3 months"] = out[doctor_col].map(avg_baseline).round(6)

    is_jan = out[monthperiod_col] == report_jan
    ...
    out.loc[~is_jan, "All Zero"] = False
    out.loc[~is_jan, "Variance >25%"] = False
    out.loc[~is_jan, "Increasing"] = False
    out.loc[~is_jan, "Decreasing"] = False
...
def validate_variance_flags(out: pd.DataFrame, product_cols: List[str]) -> None:
    ...
    if "AVG of last 3 months" in out.columns:
        av_col = pd.to_numeric(out["AVG of last 3 months"], errors="coerce").fillna(0).to_numpy(dtype=float)
        if not np.allclose(av_col, avg_baseline, rtol=1e-9, atol=1e-5):
            raise ValueError(
                "AVG of last 3 months must equal Sum of last 3 months / 3 for every row (validation failed)."
            )
...
def build_output_in_template_format(
    ...
    if i_var >= 0:
        output_fields = (
            output_fields[:i_var] + ["Sum of last 3 months", "AVG of last 3 months"] + output_fields[i_var:]
        )
    ...
    # Parse dates (use --dayfirst for DD/MM/YYYY)
    df[date_col] = pd.to_datetime(df[date_col], errors="coerce", dayfirst=dayfirst)
    if filed_date_col in df.columns:
        df[filed_date_col] = pd.to_datetime(df[filed_date_col], errors="coerce", dayfirst=dayfirst)
    ...
    if rm_key in ("latest", "auto", "max"):
        report_jan = df["_MonthPeriod"].max()
        if pd.isna(report_jan):
            raise ValueError("Could not infer report month from Date column (no valid dates).")
        baseline_months = get_baseline_months(report_jan)
    elif report_month:
        report_jan = parse_report_month(report_month)
        if report_jan is None:
            raise ValueError(f"Invalid --report-month: {report_month}. Use e.g. 2026-03, or 'latest'.")
        baseline_months = get_baseline_months(report_jan)
    else:
    ...
    if df.empty:
        raise ValueError(
            f"No data left after filtering to baseline {baseline_months} and report month {report_jan}."
        )
    ...
    print(
        f"SlotMAX Div35: Date column -> report month={report_jan} | baseline months={baseline_months}",
        flush=True,
    )
    ...
    # Aggregate same brand across slots -> max Rx
    agg = (
    ...
    # Ensure all 4 allowed brand columns exist (missing -> NaN)
...
def parse_args():
    p = argparse.ArgumentParser(description="SlotMAX Division 35 -> out_jan_div35.csv")
