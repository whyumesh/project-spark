...
"""
SlotMAX for Division 30. Same logic as generic script; 4 brands: UDILIV, COLOSPA, FLORACHAMP, EZYBIXY.
Output: Report month (Feb) in the exact schema/order of a template CSV -> out_jan_div30.csv.

What it does:
1) Only 4 brands count: UDILIV, COLOSPA, FLORACHAMP, EZYBIXY. All other brands ignored.
...
6) Pivot to product columns (COLOSPA, UDILIV, FLORACHAMP, EZYBIXY).
...
def fill_hierarchy_from_raw_territory_codes(
    ...
    *,
    ...
    # Build compact raw table for mappings
    ...
    # Designation based on territory code prefix
    ...
    # Fill codes/names only when blank
...
def add_variance_flags_octdec_vs_jan(
    ...
    For variance math only, NaN in product columns are treated as 0. Flags set only on report month rows.
    """
    out = df.copy()
    for c in product_cols:
        if c in out.columns:
    ...
    nov_period, dec_period, jan_period = (
        baseline_months[0],
        baseline_months[1],
        baseline_months[2],
    )
...
def validate_variance_flags(out: pd.DataFrame, product_cols: List[str]) -> None:
    """
    Recompute feb_total, avg_baseline, percent_diff, variance_flag from output columns and assert
    that the computed variance flag matches the output 'Variance >25%' column for every row.
    Raises with logged mismatches if any.
    """
    if out.empty or "Variance >25%" not in out.columns or "Sum of last 3 months" not in out.columns:
        return
    # Recompute Feb total per row (sum of product columns; treat NaN as 0 for consistency)
    ...
    # Same safe logic as in add_variance_flags_octdec_vs_jan
    mask_both_zero = (avg_baseline == 0) & (feb_total == 0)
    mask_baseline_zero_feb_positive = (avg_baseline == 0) & (feb_total > 0)
    ...
    if actual.dtype == bool:
        actual_bool = actual.to_numpy(dtype=bool)
    else:
        actual_bool = actual.astype(str).str.strip().str.upper().isin(("TRUE", "1", "YES", "T")).to_numpy(dtype=bool)

    mismatch = expected_variance != actual_bool
    if np.any(mismatch):
        indices = np.where(mismatch)[0]
        out_index = out.index
        for i in indices[:20]:  # log up to 20
            idx = out_index[i]
            row = out.loc[idx]
            doc = row.get("Account: Customer Code", idx)
            print(
                f"Variance mismatch row index={idx} doctor={doc}: "
                f"feb_total={feb_total[i]:.2f} avg_baseline={avg_baseline[i]:.2f} "
                f"expected_variance={expected_variance[i]} actual={actual_bool[i]}",
                file=__import__("sys").stderr,
            )
        if len(indices) > 20:
            print(f"... and {len(indices) - 20} more variance mismatches", file=__import__("sys").stderr)
        raise ValueError(
            f"Variance validation failed: {int(mismatch.sum())} row(s) have Variance >25% flag that does not match recomputed value."
        )
...
def build_output_in_template_format(
    ...
    # Parse dates (use dayfirst=True for DD/MM/YYYY common in India)
