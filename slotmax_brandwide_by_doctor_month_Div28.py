...
"""
SlotMAX for Division 28. Same logic; 4 brands: CREON, HEPTRAL SAME, VONEFI, ROWASA.
Output: Report month (Feb) in template schema -> out_jan_div28.csv.

What it does:
1) Only 4 brands count: CREON, HEPTRAL SAME, VONEFI, ROWASA. All other brands ignored.
...
6) Pivot to product columns (CREON, HEPTRAL SAME, VONEFI, ROWASA).
...
def validate_variance_flags(out: pd.DataFrame, product_cols: List[str]) -> None:
    """Recompute variance from output columns and assert it matches 'Variance >25%'. Raises if mismatch."""
    ...
    if np.any(mismatch):
        ...
        for i in indices[:20]:
            idx = out.index[i]
            print(f"Variance mismatch row index={idx}: expected={expected_variance[i]} actual={actual_bool[i]}", file=__import__("sys").stderr)
        if len(indices) > 20:
            print(f"... and {len(indices) - 20} more", file=__import__("sys").stderr)
        raise ValueError(f"Variance validation failed: {int(mismatch.sum())} row(s) mismatch.")
...
def build_output_in_template_format(
    ...
    print(
        f"SlotMAX Div28: Date column -> report month={report_jan} | baseline months={baseline_months}",
        flush=True,
    )
